...
<section class="auth-page">
  <div class="auth-card">
    <h1 class="auth-card__title">Daftar di {siteName}</h1>
    <p class="auth-card__hint">Bikin akun baru pakai email — atau pakai Google biar lebih cepat.</p>
    ...
    <form method="POST" class="auth-form" autocomplete="on">
      <label class="auth-field">
        <span>Nama</span>
        <input type="text" name="name" autocomplete="name" required minlength="2" value={form?.name ?? ''} />
      </label>
      ...
      <button class="auth-btn auth-btn--primary" type="submit">Buat Akun</button>
    </form>
...
<style>
  ...
  .auth-btn {
    ...
    gap: 8px; padding: 11px 14px; border: 1px solid transparent; border-radius: 10px;
    font-weight: 600; font-size: 14px; cursor: pointer; text-decoration: none;
    transition: filter 0.15s;
  }
  .auth-btn--primary {
    background: var(--site-primary, #f1ff32);
    color: var(--site-dark, #0a0a0a); margin-top: 4px;
  }
  .auth-btn--primary:hover { filter: brightness(0.95); }
  .auth-btn--google {
    background: #fff; border-color: #d1d5db; color: #111; margin-top: 4px;
  }
  ...
  .auth-divider::before, .auth-divider::after {
    content: ''; position: absolute; top: 50%; width: 40%; border-top: 1px solid #e5e7eb;
  }
  .auth-divider::before { left: 0; }
  .auth-divider::after { right: 0; }
  .auth-divider span { background: #fff; padding: 0 10px; position: relative; z-index: 1; }
  .auth-footer { text-align: center; font-size: 13px; color: #6b7280; margin: 20px 0 0; }
